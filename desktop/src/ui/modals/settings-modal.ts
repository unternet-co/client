...
export class SettingsModal extends ModalElement {
  ...
  constructor() {
    super();
    // Initialize properties after the element is connected
  }

  connectedCallback() {
    // Initialize properties when the element is connected to the DOM
    this.openaiApiKey = openaiService.getApiKey() || '';

    // Load cached models first, then refresh in background
